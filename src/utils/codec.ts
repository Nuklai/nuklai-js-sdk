...
class Codec {
  ...
  unpackShort(): number {
    ...
    const value = new DataView(this.buffer.buffer).getUint16(
      this.offset,
      false
    );
    this.offset += SHORT_LEN;
  ...
  unpackID(required: boolean): Id {
    ...
    if (required && this.bufferEquals(id, EMPTY_ID.toBytes())) {
  ...
  unpackAddress(): Address {
    ...
    if (this.bufferEquals(address, EMPTY_ADDRESS.toBytes())) {
  ...
  private bufferEquals(buf1: Uint8Array, buf2: Uint8Array): boolean {
